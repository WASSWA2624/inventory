# 490 — Role matrix and permission checks

**Phase** 25 · Optional team backend  |  **Depends on** [489](489-be-device-enrol.md)
...
Write one permission function per capability and the table-driven test that covers every role against every capability.
...
1. Implement administrator, project manager, reviewer and field operator per the specification.
2. Check permissions in services, never in routes, so a new route cannot forget.

## Constraints

- Obey `backend/rules/`. The ones that bite here: `backend/rules/05-security.md`, `backend/rules/09-testing.md`.
...
- [ ] Tests written and passing: The full role-by-capability matrix as a table test.
